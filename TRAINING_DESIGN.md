...
当前从零平地训练入口为 `Infantry-2027-Flat-Stable-v2`，默认 4096 环境、5000 updates。
它保留 v1 的最终命令范围、VMC、五帧历史、监督 Encoder 与 145 维 Critic 契约，但把
行为塑形恢复到同一资产上已验证的平地基线：下面列出的 16 项奖励与尺度、适中 DR、
5 秒命令重采样，并且平地阶段没有周期性推扰或向下冲击。`Flat-DR-v1` 不被覆盖，
仅保留用于复现实验。
...
全部 DR 维度从训练开始即存在。只有指令支持集在同一 optimizer、同一 run 内连续扩张；这不是多阶段 checkpoint 迁移。前 1500 次更新后固定为最终范围，余下 3500 次更新用于最终分布收敛。Flat-Stable-v2 使用已验证范围：轮/机体材料摩擦 `0.6–1.4`、恢复系数 `0.6–1.0`，base mass 加性偏移 `-1–2 kg`、惯量缩放 `0.9–1.1`、COM 三轴 `±0.02 m`、默认关节位置 `±0.03 rad`；VMC Kp/Kd 与电机强度缩放 `0.95–1.05`，动作延迟 `0–5` 个物理步。平地阶段不启用 interval push/down-impact。
...
Flat-Stable-v2 的 Critic 从平地开始就保留 77 维高度扫描，总维数为 145。平地扫描没有
策略信息增益，但它只进入训练期 Critic；保留它是为了使后续 Terrain-v2 能直接加载完整
checkpoint，而 Actor 125 维、Encoder 3 维和部署接口始终不变。
