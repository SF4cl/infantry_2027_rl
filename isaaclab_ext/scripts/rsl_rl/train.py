...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Explicit checkpoint path. Supports a flat-to-terrain resume across experiment folders.",
)
parser.add_argument(
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if agent_cfg.class_name == "InfantryOnPolicyRunner":
        runner = InfantryOnPolicyRunner(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
    ...
    # Run training.  Interpret max_iterations as the absolute target during a
    # resume, and always preserve a final/interrupted checkpoint.
    learning_iterations = agent_cfg.max_iterations
    if agent_cfg.resume:
        learning_iterations = max(0, agent_cfg.max_iterations - runner.current_learning_iteration)
        print(
            f"[INFO] Resume target iteration: {agent_cfg.max_iterations}; "
            f"remaining updates: {learning_iterations}"
        )
    try:
        runner.learn(num_learning_iterations=learning_iterations, init_at_random_ep_len=True)
    except KeyboardInterrupt:
        if runner.tot_timesteps > 0:
            runner.current_learning_iteration += 1
        interrupted_path = os.path.join(log_dir, f"model_interrupted_{runner.current_learning_iteration}.pt")
        runner.save(interrupted_path, infos={"reason": "KeyboardInterrupt"})
        print(f"[INFO] Interrupted checkpoint saved to: {interrupted_path}")
    else:
        if learning_iterations > 0:
            runner.current_learning_iteration += 1
        final_path = os.path.join(log_dir, f"model_{runner.current_learning_iteration}.pt")
        runner.save(final_path, infos={"reason": "completed"})
        print(f"[INFO] Final checkpoint saved to: {final_path}")
